...
> Canonical reference for the YODY design system: brand, tokens, architecture, the four surfaces, principles, the Tailwind v4 layer, the component library, and how to consume it in a real repo.
>
...
> Brand identity + production-ready component library for **YODY**, a Vietnamese fashion retail brand.
>
...
**Stack:** Tailwind v4, shadcn-standard — semantic role tokens + a tokenized type scale, an `@apply` component layer, and an open-code React + Radix registry. YODY brand tokens stay available alongside the shadcn roles.
...
<head>
  <!-- 1 · Tokens (source of truth: :root + 4 surface adapters).
       styles.css is the root single-link entry — it @imports colors_and_type.css,
       so you can link either one. -->
  <link rel="stylesheet" href="styles.css" />

  <!-- 2 · Tailwind v4 + DS component layer.
       CDN / no-build → inline the bundle (browser build only compiles inline):
         paste the contents of tailwind/yody.components.css inside the style block below.
       Production build (Vite/PostCSS) → use tailwind/yody.tailwind.css as the entry instead. -->
  <script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script>
  <style type="text/tailwindcss">/* … paste tailwind/yody.components.css here … */</style>

  <!-- 3 · Add the composite kits you need (token-driven CSS modules) -->
  <link rel="stylesheet" href="ui_kits/yody-app/index.css" />
  <link rel="stylesheet" href="ui_kits/yody-fashion/index.css" />
  <link rel="stylesheet" href="ui_kits/yody-ai/index.css" />
  <link rel="stylesheet" href="ui_kits/yody-collab/index.css" />
</head>
...
| **III · UI kits** | `ui_kits/yody-{app, fashion, ai, collab}/*.css` — composite components | Consumes adapter tokens |
...
|---|---|---|---|---|---|---|
| **Marketing** | yody.vn landing, careers, brand story, campaign | 18px / 1.6 | 88–120px | 1200px | rich (scroll-reveal OK) | bold |
| **App** | Back-office, dashboards, vendor portal, internal mobile | **14px / 1.5** | h1 28px | 100% fluid | instant (state-only) | signal |
| **Portal** *(default)* | Executive briefings, reports, internal microsites | 18px / 1.7 | 80–120px | 1280px | 1 ambient/page | narrative |
| **Store** | POS terminals, signage, fitting-room tablet (v3.1) | 18px / 1.45 | signage 96–160px | variable | ambient-loop / none | high-contrast |
...
REGISTRY.md                   Install workflow — npx shadcn add (YODY items by url/@yody namespace + upstream auto-branded)
public/r/                     Built registry — one installable JSON per item (registry-item schema, file contents inlined)
index.html                    Repo home — human-friendly intro/guide + hub linking specimens, examples & docs
styles.css                    Root single-link entry — link this one file for the token foundation (@imports colors_and_type.css)
...
├── yody.tailwind.css         Build entry: @import "tailwindcss" + @theme roles + type scale + @apply component layer
├── yody.components.css       No-Preflight bundle (theme + utilities + @apply components + typography) — inline this in CDN/no-build
├── yody.theme.css            Tokens-only theme bridge (utilities, no component layer)
...
├── yody-app/                 Shell · top-bar · side-rail · data-table · filter · cmd-K · charts  [CORE · generic]
├── yody-ai/                  Composer · halo field · confidence meter · stream · suggestion · panel · disclosure (v2)  [CORE · generic]
├── yody-collab/              Comment · annotation · approval · activity · presence                [CORE · generic]
└── yody-fashion/             Swatch · BOM table · size matrix · style card · tech-pack A4         [SCOPED · fashion domain]

registry/                     React component source (Tier 3 · open-code, shadcn-installable via registry.json)
├── lib/utils.ts              cn() — clsx + tailwind-merge
├── hooks/use-mobile.ts       viewport < 768px hook (used by sidebar)
└── ui/                       47 components (.tsx) — CVA + Radix/cmdk/vaul/embla/recharts/react-day-picker/react-hook-form, YODY-branded via role tokens
                              · core      : button · badge · input · label · card · switch · dialog · tabs
                              · form       : textarea · checkbox · radio-group · select · slider · toggle · toggle-group · input-otp · form
                              · overlay    : tooltip · popover · hover-card · dropdown-menu · context-menu · menubar · alert-dialog · sheet · drawer · command · combobox
                              · disclosure : accordion · collapsible
                              · nav        : breadcrumb · pagination · navigation-menu
                              · display    : table · alert · progress · skeleton · avatar · separator · aspect-ratio · scroll-area · calendar · carousel · chart · sonner
...
├── components/              ONE specimen per component (shadcn-docs style, 49 cards)               [Components]
│   ├── <name>.html          44 shadcn primitive specimens (accordion · alert · … · tooltip), 1:1 with registry/ui
│   └── forms · overlays · state · react · react-radix                                             (YODY CSS-layer + live-React specimens)
├── blocks/                  Composite patterns (shadcn-blocks style)                                [Blocks]
│   ├── header · data-viz · kpi-cards                                                              (data-viz, kpi-cards, app-* also @startingPoint)
│   └── app-{chrome,data-table,charts} · ai-vocabulary (v1, legacy) · ai-vocabulary-v2 (canonical) · collab-components · fashion-atoms
└── brand/                   logo-lockups · iconography · identity                                   [Brand]
...
└── kpi-network/              KPI network graph — 3000-node canvas viz (app surface; a DS consumer)
```

---

## Token authoring — the `@kind` annotation (for coding agents)

The build compiler classifies every `:root` / `[data-surface]` custom property as **color · spacing · radius · shadow · font** from its name or value, and emits an adherence lint set from it. Tokens whose value matches none of those — easing curves, durations, keyword values (`ambient`, `narrative`…), and gradients — can't be auto-classified and **must be annotated**, or the design-system check flags them:

- Put `/* @kind <kind> */` **on the same line, right after the `;`** of the declaration. The label must be exactly one of `color | spacing | radius | shadow | font | other` — nothing else inside that comment (any prose goes in a separate comment). An annotation on its own line is ignored.
- Use `/* @kind other */` for motion/easing/duration/keyword tokens (`--ease-*`, `--dur*`, `--motion-*`, `--motion-budget`, `--accent-freq`); `/* @kind color */` for gradient tokens (`--grad-*`).
- A token redefined across surface adapters needs the annotation on **each** declaration.
- Component-scoped custom properties (e.g. `.chart-svg .s1 { --series: … }`) are local theming hooks, not global tokens — annotate them with `/* @kind … */` too so they aren't reported as stray tokens.

After editing tokens, re-run the design-system check (`/design-sync` in Claude Code) until it reports clean. Note: the `registry/ui/*.tsx` components import npm packages (Radix, lucide, cva, clsx, tailwind-merge) — the in-browser preview bundler can't resolve them, so they only bundle correctly via `/design-sync`; this is expected and not a reason to strip the registry.
...
- `tailwind/yody.tailwind.css` (+ `yody.components.css` / `yody.theme.css`) — Tailwind v4 layer: semantic role tokens + type scale + `@apply` component layer (buttons, typography, forms, overlays, identity, data-viz, state — formerly `foundation/*.css`)
- `ui_kits/yody-app` · `yody-ai` · `yody-collab` — generic, cross-product kits
- `fonts/` (Be Vietnam Pro) · `assets/yody-logo.png`
...
- `ui_kits/yody-fashion/` — fashion **domain** kit (specimen: `preview/components/kits/fashion-atoms.html`)
...
This makes the YODY DS a **shadcn-compatible registry**: components install with
...
```
registry.json            Source manifest (54 items: theme · style · utils · use-mobile hook · 47 UI · 4 kits)
...
components.json          shadcn config (css → globals.css, tsx, aliases, @yody namespace)
```
...
### 2 · YODY components, by namespace
With the `@yody` entry in `components.json` `registries`:
```bash
npx shadcn@latest add @yody/button @yody/card @yody/tabs
```

### 3 · Upstream shadcn components — auto-branded fallback
The YODY registry now ships **47 components** open-code (`@yody/<name>`), so prefer those.
For anything not yet authored, pull the upstream item — because `tailwind/globals.css`
defines the role tokens, it renders YODY-branded (navy primary, iris ring, `--radius`)
with no edits:
```bash
npx shadcn@latest add @yody/select @yody/calendar @yody/chart   # YODY open-code (preferred)
npx shadcn@latest add <upstream-name>                           # auto-branded fallback for un-authored items
```
Every YODY item is open-code: edit after install for specifics (pill radius, gold/status
variants), exactly as `registry/ui/button.tsx` customizes the stock button.
...
npx shadcn@latest add @yody/button   # or a URL, or upstream names
```
...
<style type="text/tailwindcss"> …nội dung yody.tailwind.css… </style>        <!-- 3 -->
```
...
Dùng thẳng `yody.tailwind.css` làm entry — `@import "tailwindcss"` + `@theme` + `@apply`
...
- `foundation/*.css` đã **retired** (v3.7) — component layer giờ sống trong `@apply` (`yody.tailwind.css` / inline `yody.components.css`). Specimen + examples không còn `<link foundation/index.css>`.

## Canonical theme — `globals.css` (shadcn shape)

`tailwind/globals.css` là file theme **canonical** theo đúng shape shadcn/ui:
- `:root` — **role layer**: semantic tokens (`--background`, `--foreground`, `--card`, `--primary`, `--secondary`, `--muted`, `--accent`, `--destructive`, `--input`, `--ring`, `--chart-1..5`, `--sidebar*`, `--radius`) **alias lên token YODY** (`colors_and_type.css` vẫn là nguồn giá trị thô).
- `.dark` — override cùng bộ role token (opt-in qua class `.dark`; YODY mặc định light).
- `@theme inline` — map role → utility (`bg-background`, `text-muted-foreground`, `border-border`, `ring-ring`, `bg-chart-1`, `bg-sidebar`…) + brand palette + fonts + radius scale (sm/md/lg/xl explicit YODY, 2xl/3xl/4xl = `calc(var(--radius)*…)`) + type scale.
- `@layer base` — `* { @apply border-border outline-ring/50 }` + `body { @apply bg-background text-foreground }`.

**Quan hệ file:** `globals.css` = nguồn theme chuẩn (build mode: Vite/PostCSS import được). CDN no-build không `@import` file rời được → bản mirror đã inline nằm trong `yody.components.css` (giữ đồng bộ với globals.css). **Dark mode chỉ chạy ở build-mode/globals** (CDN bundle hiện không kèm `.dark`).
...
- ✅ **Component parity shadcn** (Tier 3): registry **47 component** open-code (`registry/ui/*.tsx` + `use-mobile` hook), 1:1 `public/r/*.json` + served index — cài qua `@yody/<name>`. Preview tách **một specimen/component** (`preview/components/<name>.html`, kiểu trang docs shadcn).
- ✅ Bridge Tailwind v4: `yody.tailwind.css` (full component layer) + `yody.theme.css` (tokens-only, no-Preflight).
...
- ◻ Kits (`yody-app/ai/collab/fashion`) giữ dạng CSS module token-driven, đã Tailwind-enabled — không port composite sang @apply (đã token-driven, không lợi ích thêm).
...
| **Applies to** | All surfaces. |
| **Why** | The adapter pattern only works if the surface is announced. Without the attribute, app screens accidentally use portal's 18px body and break density. |
...
## VI · ≥18px body (projector legibility)

| | |
|---|---|
| **Rule** | Portal body text minimum is 18px / 1.7 line-height. Lede is 22–24px. Labels 14px floor. |
| **Why portal-only** | A 14″ laptop at arm's length needs ~14px; a 1080p projector at 3m needs ~18px. The portal is consumed in *both* modes, so we calibrate for the harder. App surface has the opposite problem — desk, 30 cm distance, 14px is comfortable. |
| **Operationalisation** | `--type-body: 400 18px/1.7 var(--font-body)` in `:root` (which is the portal default). The marketing and app adapters override this. |
| **Tension** | This is the principle most likely to be misapplied. Resist the impulse to scale every YODY interface up. |
...
```
[s-eyebrow]  mono 12px · 0.32em tracking · flanking dashes
...
> These are **products that consume** the YODY Design System — applications and product mockups built *with* the foundation + kits. They are kept as reference implementations, but they are **out ofscope** for the design system itself.
>
...
| `kpi-network/` | KPI network graph — interactive 3000-node canvas visualization (data clustered org → khối → phòng ban → KPI, with hover-highlight, drag, zoom/pan, filter, detail panel). | new | App-surface product screen built on the DS (canvas engine + chrome), not a reusable DS primitive. |

## Paths still resolve

Both folders sit at depth 2 from repo root, same as their old locations, so the existing
`../../colors_and_type.css`, `../../tailwind/yody.components.css`, `../../ui_kits/*` references are unchanged.
