...
function LensSwitcher({ lens, onLens }) {
  return (
    <div className="lensbar shell">
      <div className="lensseg" role="group" aria-label="Lăng kính">
        <button aria-pressed={lens === 'bsc'} onClick={() => onLens('bsc')}>
          <span className="gl"><window.Icon name="target" size={14} /></span> BSC Outcome
        </button>
        <button aria-pressed={lens === 'agile'} onClick={() => onLens('agile')}>
          <span className="gl"><window.Icon name="gitBranch" size={14} /></span> Agile Delivery
        </button>
      </div>
    </div>
  );
...
function CommandBar({ filter, setFilter, total, shownCount, perspWeight }) {
  ...
  useEffect(() => {
    const el = stuckRef.current; if (!el) return;
    const onScroll = () => { el.classList.toggle('stuck', el.getBoundingClientRect().top <= 61); };
  ...
  return (
    <div className="cmdbar shell" ref={stuckRef}>
      <div className="cmd-row">
        <span className="cmd-lab">Góc nhìn</span>
        ...
        <div className="wk-select" title="W2–W4 chưa có dữ liệu — mở dần theo tuần">
          <window.Icon name="clock" size={14} color="var(--fg-3)" />
      ...
      <div className="cmd-row">
        <span className="cmd-lab">Team</span>
        <div className="chips" role="group" aria-label="Lọc theo team">
          {teams.map((t) => {
            const on = filter.teams.indexOf(t) >= 0;
            return (
              <button key={t} className="chip" aria-pressed={on} onClick={() => toggleTeam(t)}>
      ...
      <div className="cmd-row">
        <span className="cmd-lab">Trạng thái</span>
      ...
      {filter.perspective !== 'all' && (
        <div className="subview">
          ...
          Đang xem: <b style={{ color: 'var(--brand)', fontWeight: 700 }}>{perspLabel}</b> · {shownCount} chỉ tiêu · {perspWeight}% trọng số nhóm (thanh sức khỏe vẫn tính trên 100% toàn cảnh)
        </div>
      )}
    </div>
  );
...
function VerdictStrip({ health }) {
  ...
  return (
    <section className="verdict" data-screen-label="verdict-strip">
      <div className="verdict-main">
        <div className="seal" />
        <span className="verdict-eyebrow"><window.Icon name="diamond" size={11} color="var(--gold)" /> Kết luận · Tracking tuần 1 · {D.TRACKING_META.asOf}</span>
        <p className="verdict-body">
          Tuần 1 mở tháng <em>tích cực</em> — <b className="tnum">{w['on-track'].weight}/{total}%</b> trọng số đúng tiến độ và tiến độ lũy kế đang dẫn trước lịch (W1 ≈ 23% thời gian). Rủi ro dồn vào một điểm: <b>OMEGA</b> (Organic Traffic &amp; CR off-track) và <b>NSHP</b> thiếu người.
        </p>
        <div className="verdict-meta">
          <span>12 chỉ tiêu · 4 perspective</span>
          <span>Trọng số 100%</span>
          <span>Nguồn: PITSTOP 6 team</span>
        </div>
      </div>
      <div className="verdict-side">
        <div className="health-head">
          <div className="health-num">
            <span className="v3-kpi v3-num-grad tnum">{score}</span>
            <span className="v3-kpi v3-num-grad" style={{ fontSize: '.42em' }}>/100</span>
          </div>
          <span className="health-label">Sức khỏe KPI<br /><span className="health-info" title={HEALTH_TIP}>cách tính ⓘ</span></span>
        </div>
        <div>
          ...
          <div className="hbar-legend">
            {HBAR_ORDER.map((s) => (
              <span className="lg" key={s.key}>
                <i style={{ background: s.fill }} /><b>{s.label}</b>
                <span className="v3-mono tnum">{w[s.key].weight}% · {w[s.key].count}</span>
              </span>
            ))}
          </div>
        </div>
        <div className="statpills">
          {HBAR_ORDER.map((s) => (
            <div className="statpill" key={s.key} style={{ borderLeftColor: s.fill }}>
              <div className="top">
                <span className="cnt tnum" style={{ color: s.text }}>{w[s.key].count}</span>
                <span className="lab" style={{ color: s.text }}>{s.label.split(' ')[0]}</span>
              </div>
              <div className="wt tnum">{w[s.key].weight}% trọng số</div>
            </div>
          ))}
        </div>
      </div>
